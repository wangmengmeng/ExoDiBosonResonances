...
void btagveto_significance()
{
      TString inputpath = "/afs/cern.ch/work/m/mwang/public/EDBR/trees/productionv1_1110_Pt200/fullrange/";
...
//	TString cut = "ttbar_control_significance_clean";
      TString cut = "btagveto_significance_gg";
	bool clean = true;
...
//    sigSamples.push_back("MWp_1800_xwh");
...
//    sigSamples.push_back("BulkG_WW_lvjj_c0p2_M2000_xww");
	...
	TH1F * h1 = new TH1F (cut,cut,2,0,2);
			...
			for(int ivec=0;ivec<nCands;ivec++){
				...
				if(filled==0)
				{
                    ...
                    total = total + actualWeight;
                                      ...
                                      if(nbtagsM[ivec]==0)pass1=pass1+actualWeight;//M
                                      if(nbtagscleanM[ivec]==0)pass2=pass2+actualWeight;//cleanM
//                                      if(nbtagsM[ivec]>=1)pass3=pass3+actualWeight;//M
...
//			sigeff6=eff6;
	...
	c1->SetGridy(1);
        h1->Fill("M",sigeff1/(1+sqrt(totalB1)));    
        h1->Fill("cleanM",sigeff2/(1+sqrt(totalB2)));          
/*
