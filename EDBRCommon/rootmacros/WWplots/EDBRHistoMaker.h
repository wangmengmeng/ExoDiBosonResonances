...
class EDBRHistoMaker {
	...
	public :
		...
		float nsubjetbtagL[99]; //[nCands]
		float nsubjetbtagM[99]; //[nCands]
		float nsubjetbtagT[99]; //[nCands]
		float nfatjetbtagL[99]; //[nCands]
		float nfatjetbtagM[99]; //[nCands]
		float nfatjetbtagT[99]; //[nCands]
...
void EDBRHistoMaker::Init(TTree *tree)
{
	...
	fChain->SetBranchAddress("nsubjetbtagL",nsubjetbtagL);
	fChain->SetBranchAddress("nsubjetbtagM",nsubjetbtagM);
	fChain->SetBranchAddress("nsubjetbtagT",nsubjetbtagT);
	fChain->SetBranchAddress("nfatjetbtagL",nfatjetbtagL);
	fChain->SetBranchAddress("nfatjetbtagM",nfatjetbtagM);
	fChain->SetBranchAddress("nfatjetbtagT",nfatjetbtagT);
...
void EDBRHistoMaker::createAllHistos() {
	...
	hs.setHisto("prunedmass",50, 0, 250); //16, 80, 160); //50,0, 250); //25,35,160); //40,160); // 5 GeV bins
	...
	hs.setHisto("nsubjetbtagL",5,0,5);
	hs.setHisto("nsubjetbtagM",5,0,5);
	hs.setHisto("nsubjetbtagT",5,0,5);
	hs.setHisto("nfatjetbtagL",5,0,5);
	hs.setHisto("nfatjetbtagM",5,0,5);
	hs.setHisto("nfatjetbtagT",5,0,5);
	...
	if(isZZchannel_==0)//WW channel, use special binning in sync with FNAL
	{
		...
		hs.setHisto("ptjet1",30,300,600);
		hs.setHisto("etajet1",30,-2.5,2.5);
		hs.setHisto("phijet1",40,-3.14,3.14);
		hs.setHisto("mJJ",28,0,140);
		hs.setHisto("prunedmass",50, 0, 250); //16, 80,160); //50, 0, 250); //25,35,160); //135);//PAS
	...
	hmjmzz=new TH2D("h_mj_vs_mzz","Correlation plot M_{J} vs M_{ZZ}; M_{ZZ} [GeV]; M_{J} [GeV]",280,200,3000,25,35.0,160.0); //135.0);
...
bool EDBRHistoMaker::eventPassesCut(int i, double ptZll_threshold, double ptlep1_threshold ) {
	...
	bool passesMZZ=mZZ[i]>=600.0&&mZZ[i]<3000.0;
...
void EDBRHistoMaker::Loop(std::string outFileName){
		...
		for(int ivec=0;ivec<nCands;ivec++){
			...
			if(eventPassesCut(ivec, 80, 20)){
				...
				if(isZZchannel_==0)//WW channel, veto second loose lepton
				{
					...
					// change to nbtagscleanL==0 for WH analysis
					if(isTTcontrol_==false&&nbtagscleanL[ivec]==0) ; 
					else if(isTTcontrol_==true&&nbtagscleanM[ivec]>=1) ;//b cut - ttbar control region - to sync we use clean M
					else continue;

					if(ptjet1[ivec]>200);
					else continue;
					if(nsubjetbtagL[ivec]>=2);
					else continue;
				...
				if(filled==0)
				{
					...
					(theHistograms["nsubjetbtagL"])->Fill(nsubjetbtagL[ivec],actualWeight);
					(theHistograms["nsubjetbtagM"])->Fill(nsubjetbtagM[ivec],actualWeight);
					(theHistograms["nsubjetbtagT"])->Fill(nsubjetbtagT[ivec],actualWeight);
					(theHistograms["nfatjetbtagL"])->Fill(nfatjetbtagL[ivec],actualWeight);
					(theHistograms["nfatjetbtagM"])->Fill(nfatjetbtagM[ivec],actualWeight);
					(theHistograms["nfatjetbtagT"])->Fill(nfatjetbtagT[ivec],actualWeight);
	...
	if(isZZchannel_==0)//WW channel, change the names, which will be the plot lable
	{
		...
		(theHistograms["ptZjj"])->SetName("h_ptHjj");
		(theHistograms["yZll"])->SetName("h_yWL");
		(theHistograms["yZjj"])->SetName("h_yHjj");
		(theHistograms["mLL"])->SetName("h_mWL");
		(theHistograms["mZZ"])->SetName("h_mWH");
