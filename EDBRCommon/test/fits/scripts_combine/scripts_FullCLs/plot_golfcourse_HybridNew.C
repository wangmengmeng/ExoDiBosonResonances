...
void plot_golfcourse_HybridNew(bool unblind, char* width){
 ...
 //read in theoretical values from text files
  ...
  // cout<<"Plotting"<<endl;
  double fr_left=590.0, fr_down=0.0005,fr_right=2520.0,fr_up=1.0;
  ...
  /*
  TFile *fUnMPlus=new TFile("HybridNewCLs_UnmatchedPlus_TGraph.root","READ");
 ...
 if(useNewStyle){
   ...
   char fnam[50];
   if(width != 0){
     sprintf(fnam,"EXOZZ_2l2q_UL_HybridNew_%s.root",width);
     cMCMC->SaveAs(fnam);
     sprintf(fnam,"EXOZZ_2l2q_UL_HybridNew_%s.eps",width);
     cMCMC->SaveAs(fnam);
     sprintf(fnam,"EXOZZ_2l2q_UL_HybridNew_%s.png",width);
     cMCMC->SaveAs(fnam);
     gPad->SetLogy();
     sprintf(fnam,"EXOZZ_2l2q_UL_HybridNew_%s_log.eps",width);
     cMCMC->SaveAs(fnam);
     sprintf(fnam,"EXOZZ_2l2q_UL_HybridNew_%s_log.png",width);
     cMCMC->SaveAs(fnam);
   }
   else{
     cMCMC->SaveAs("EXOZZ_2l2q_UL_HybridNew.root");
     cMCMC->SaveAs("EXOZZ_2l2q_UL_HybridNew.eps");
     cMCMC->SaveAs("EXOZZ_2l2q_UL_HybridNew.pdf");
     cMCMC->SaveAs("EXOZZ_2l2q_UL_HybridNew.png");
     gPad->SetLogy();
     cMCMC->SaveAs("EXOZZ_2l2q_UL_HybridNew_log.eps");
     cMCMC->SaveAs("EXOZZ_2l2q_UL_HybridNew_log.pdf");
     cMCMC->SaveAs("EXOZZ_2l2q_UL_HybridNew_log.png");
  // cMCMC->SaveAs("ClsLimit_1fb.png");
   }

   if(width==0)
     sprintf(fnam,"HybridNewCLs_TGraph.root");
   else
     sprintf(fnam,"HybridNewCLs_TGraph_%s.root",width);
