...
def processSubsample(file):
    ...
    for event in events:
        ...
        eff_lepton2=1.
        if flavor=="ele":
            bin1 = histo_eff_ele.FindBin(genlepton1p4.pt(),abs(genlepton1p4.eta()))
            bin2 = histo_eff_ele.FindBin(genlepton2p4.pt(),abs(genlepton2p4.eta()))
            eff_lepton1 = histo_eff_ele.GetBinContent(bin1)
            eff_lepton2 = histo_eff_ele.GetBinContent(bin2)
        if flavor=="mu":
            bin1 = histo_eff_mu.FindBin(genlepton1p4.pt(),abs(genlepton1p4.eta()))
            bin2 = histo_eff_mu.FindBin(genlepton2p4.pt(),abs(genlepton2p4.eta()))
            eff_lepton1 = histo_eff_mu.GetBinContent(bin1)
            eff_lepton2 = histo_eff_mu.GetBinContent(bin2)

        bin = histo_eff_jet.FindBin(genjetp4.pt(),abs(genjetp4.eta()) )
        eff_jet = histo_eff_jet.GetBinContent(bin)
        
        nsel += eff_jet*eff_lepton1*eff_lepton2

    print "passing " + str(nsel) + " events of " + str(nevent) 
    return nevent,nsel
...
def main():
    ...
    parser.add_argument('-b','--bosonflavor',  help='boson flavor, ZZ or WW, default: ZZ'                             , default = "ZZ" )                                      
    ...
    for file in filelist:
        print 'File is ',file
        n,s = processSubsample(file)
        ntot+=n
        stot+=s

    print "estimated efficiency is " + str(stot/ntot)
